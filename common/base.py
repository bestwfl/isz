...
def get_hosts_env():
    ...
    try:
        ...
        # consoleLog('The hosts file is empty or line 4 is empty')
        env = None
    hosts.close()
    return env
...
def get_conf(section, option, valueType=str):
    if section == 'testCondition':
        ...
        if not get_conf('testCondition', 'test') == get_hosts_env():
            if get_hosts_env() in ['test', 'mock']:
                set_conf('testCondition', test=get_hosts_env())  # 如果hosts是预发或者测试，直接改config为预发和测试
        if get_conf('testCondition', 'test') == 'test':
            section = section + '_test'
...
class Base(object):
    ...
    def __init__(self):
        ...
        if self.instanceCount == 1:
            ...
            # self.input_text(loginPage.userNameInput, '18815286582')
            # self.input_text(loginPage.passWordInput, 'ceshi123456')
    ...
    def open(self, url, loc, havaFrame=False):
        """
        重写打开地址函数
        :param url: 目标地址
