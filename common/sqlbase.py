...
conn = pymysql.connect(host=get_conf('db', 'host'),
...
def serach(sql, needConvert=True, oneCount=True, research=False, nullLog=True):
    ...
    try:
        ...
        if needConvert:
            if value is None:
                if research:  # 如果查询无结果默认隔3秒再查三次
                    for i in range(3):
                        time.sleep(3)
                        # consoleLog(u'第 %s 次查询无结果' % str(i + 1))
                        value = serach(sql, oneCount=oneCount, needConvert=needConvert, nullLog=False)
                        if value:
                            break
                    return value
                if nullLog:
                    consoleLog(u'SQL返回值为空!\nSQL:%s' % sql)
                return []
            else:
